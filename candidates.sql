...
CREATE TABLE candidate (
    ...
    status varchar (25) NOT NULL check (status in ('CLEAR', 'PENDING')),
    adjucation varchar (25) DEFAULT '-' NOT NULL,
...
CREATE TABLE COUNT_SEARCHES(
    candidate_id serial NOT NULL,
    search varchar (50) NOT NULL,
    status varchar (25) NOT NULL  check (status in ('CLEAR', 'PENDING')) REFERENCES candidate(status),
    join_date date NOT NULL,
...
CREATE TABLE ADVERSE_ACTIONS(
   candidate_name varchar (50) NOT NULL,
   status varchar (25) NOT NULL check (status in ('SCHEDULED', 'PENDING' ,'DISPUTE','CANCELLED','UNDELIVERABLE')), 
  prenotice_date date NOT NULL,
  postnotice_date date NOT NULL,
